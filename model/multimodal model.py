...
class HSCANS(nn.Module):
    """Fixed-path spatial scan (identical to single-modal version)."""
    def __init__(self, size=16, dim=2, scan_type='scan'):
            ...
            if HAS_HILBERT:
                ...
                for loc in locs:
                    ...
                    for j in range(dim):
                        loc_flat += loc[j] * (l ** j)
                    ret.append(loc_flat)
                locs_flat = np.array(ret, dtype=np.uint64)
            else:
                indexes = indexes.reshape(size, size)
                for i in np.arange(1, size, step=2):
                    indexes[i, :] = indexes[i, :][::-1]
        ...
        self.index_flat = nn.Parameter(
            torch.LongTensor(locs_flat).unsqueeze(0).unsqueeze(1), requires_grad=False)
        self.index_flat_inv = nn.Parameter(
            torch.LongTensor(locs_flat_inv).unsqueeze(0).unsqueeze(1), requires_grad=False)
...
class PotentialFieldScanner(nn.Module):
    """Generates [B, 1, H, W] potential map for Dt modulation."""
...
class PotentialDtSS2D(nn.Module):
    """SS2D with potential-field Dt modulation (identical to single-modal version)."""
    def __init__(self, d_model, d_state=16, expand=2,
        ...
        self.conv2d   = nn.Conv2d(self.d_inner, self.d_inner, 3, 1, 1, groups=self.d_inner)
        self.act      = nn.SiLU()

        self.x_proj_weight = nn.Parameter(torch.stack([
            nn.Linear(self.d_inner, self.dt_rank + self.d_state * 2, bias=False).weight
            for _ in range(self.K)], dim=0))

        dt_projs = [self._dt_init(self.dt_rank, self.d_inner) for _ in range(self.K)]
        ...
        A_log = torch.log(repeat(torch.arange(1, d_state + 1, dtype=torch.float32),
        ...
        self.potential_scanner = PotentialFieldScanner(self.d_inner, kernel_size=potential_kernel)
    ...
    def forward_core(self, x, potential_bias):
        ...
        if K >= 4:
            xs.append(self.scans.encode(x.transpose(2, 3).contiguous().view(B, -1, L)))
        if K >= 8:
            x_rot = torch.rot90(x, k=1, dims=(2, 3)).contiguous()
            xs.append(self.scans.encode(x_rot.view(B, -1, L)))
            xs.append(self.scans.encode(x_rot.transpose(2, 3).contiguous().view(B, -1, L)))
        ...
        dts_raw, Bs, Cs = x_dbl.split([self.dt_rank, self.d_state, self.d_state], dim=2)
        dts = torch.einsum('b k r l, k d r -> b k d l', dts_raw, self.dt_projs_weight)

        pb_flat = potential_bias.view(B, -1, L)
        pb_h = self.scans.encode(pb_flat)
        pb_list = [pb_h, torch.flip(pb_h, dims=[-1])]
        if K >= 4:
            pb_v = self.scans.encode(potential_bias.transpose(2, 3).contiguous().view(B, -1, L))
            pb_list += [pb_v, torch.flip(pb_v, dims=[-1])]
        if K >= 8:
            pb_rot = torch.rot90(potential_bias, k=1, dims=(2, 3)).contiguous()
            pb_r   = self.scans.encode(pb_rot.view(B, -1, L))
            pb_rt  = self.scans.encode(pb_rot.transpose(2, 3).contiguous().view(B, -1, L))
            pb_list += [pb_r, torch.flip(pb_r, dims=[-1]), pb_rt, torch.flip(pb_rt, dims=[-1])]
        ...
        if K >= 4:
            ys.append(self.scans.decode(out_y[:, 1]).view(B, -1, W, H).transpose(2, 3).contiguous().view(B, -1, L))
            ys.append(self.scans.decode(inv_y[:, 1]).view(B, -1, W, H).transpose(2, 3).contiguous().view(B, -1, L))
        if K >= 8:
            ...
            ys.append(torch.rot90(self.scans.decode(out_y[:, 3]).view(B, -1, W, H).transpose(2, 3), k=3, dims=(2, 3)).contiguous().view(B, -1, L))
            ys.append(torch.rot90(self.scans.decode(inv_y[:, 3]).view(B, -1, W, H).transpose(2, 3), k=3, dims=(2, 3)).contiguous().view(B, -1, L))
    ...
    def forward(self, x):
        ...
        dt_bias_map = mod_scale * self.potential_to_dt(potential)
...
class PotentialDtMixer(nn.Module):
    def __init__(self, dim, size=8, scan_type='scan',
        ...
        self.mamba = PotentialDtSS2D(
            ...
            num_direction=num_direction, size=size, scan_type=scan_type)
    ...
    def forward(self, x):
        ...
        return x + torch.sigmoid(self.mamba_gate) * out
...
class AdaptiveBiModalFusion(nn.Module):
    """
    Adaptive weighted fusion for a pair of same-dimension feature maps.

    Uses Squeeze-and-Excitation to predict per-sample modality weights:
      1. Global average pooling over spatial dims
      2. Small MLP predicts two logits
      3. Softmax normalizes (rgb_w + dep_w = 1)
      4. Weights broadcast and multiply each branch

    Args:
        hidden_size (int): number of input channels
        bottleneck_ratio (int): MLP bottleneck compression ratio (default 8)
    """
    def __init__(self, hidden_size: int, bottleneck_ratio: int = 8):
        super().__init__()
        mid = max(hidden_size // bottleneck_ratio, 16)
        ...
        nn.init.xavier_normal_(self.gate_mlp[-1].weight)

    def forward(self, rgb_feat: torch.Tensor, dep_feat: torch.Tensor):
        """
        Args:
            rgb_feat: [B, C, H, W]
            dep_feat: [B, C, H, W]
        Returns:
            rgb_out:  [B, C, H, W]
            dep_out:  [B, C, H, W]
            weights:  [B, 2]  (rgb_w, dep_w)
        """
        rgb_gap  = rgb_feat.mean(dim=[2, 3])
        dep_gap  = dep_feat.mean(dim=[2, 3])
        combined = torch.cat([rgb_gap, dep_gap], dim=1)
        logits   = self.gate_mlp(combined)
        weights  = torch.softmax(logits, dim=1)

        rgb_w = weights[:, 0].view(-1, 1, 1, 1)
        dep_w = weights[:, 1].view(-1, 1, 1, 1)
        return rgb_feat * rgb_w, dep_feat * dep_w, weights


class ReconstructiveSubNetwork(nn.Module):
    """
    Bi-modal reconstructive network with adaptive fusion.

    forward() supports two calling conventions:
      A) model(rgb_3ch, depth_3ch)
      B) model(cat_6ch)
    """
    def __init__(self, in_channels=3, out_channels=3, base_width=128,
        ...
        self.decoder   = PotentialDtDecoder(base_width, out_channels=out_channels,
                                             scan_type=scan_type, num_direction=num_direction)
        self.decoder_d = PotentialDtDecoder(base_width, out_channels=out_channels,
                                             scan_type=scan_type, num_direction=num_direction)
        ...
        self.mlp1 = nn.Sequential(
            nn.Conv2d(base_width * 2, self.hidden_size, kernel_size=1),
            nn.BatchNorm2d(self.hidden_size), nn.LeakyReLU(0.2))
        self.mlp2 = nn.Sequential(
            nn.Conv2d(base_width * 3, self.hidden_size, kernel_size=1),
            nn.BatchNorm2d(self.hidden_size), nn.LeakyReLU(0.2))
        ...
        self.mlp3 = nn.Sequential(
            nn.Conv2d(self.hidden_size * 4, self.hidden_size, kernel_size=1),
            nn.BatchNorm2d(self.hidden_size), nn.LeakyReLU(0.2),
            nn.Conv2d(self.hidden_size, self.hidden_size1, kernel_size=1),
            nn.BatchNorm2d(self.hidden_size1), nn.LeakyReLU(0.2),
            nn.Conv2d(self.hidden_size1, self.hidden_size2, kernel_size=1),
            nn.BatchNorm2d(self.hidden_size2), nn.LeakyReLU(0.2))

        self.fusion1 = AdaptiveBiModalFusion(self.hidden_size)
        self.fusion2 = AdaptiveBiModalFusion(self.hidden_size)

        if train_model:
            self.apply(init_weight)
            self._reinit_mamba_zeros()

    def _reinit_mamba_zeros(self):
        """
        Force zero-init on all Mamba out_proj and potential_to_dt weights.
        Must be called after any apply(some_init_fn) to prevent overwriting.
        In trainer.py, call model._reinit_mamba_zeros() after model.apply(weights_init).
        """
        for dec in [self.decoder, self.decoder_d]:
            nn.init.zeros_(dec.mamba1.mamba.out_proj.weight)
            nn.init.zeros_(dec.mamba2.mamba.out_proj.weight)
            nn.init.zeros_(dec.mamba1.mamba.potential_to_dt.weight)
            nn.init.zeros_(dec.mamba2.mamba.potential_to_dt.weight)

    def forward(self, inputs, depth=None):
        if depth is not None:
            x, d = inputs, depth
        else:
            assert inputs.shape[1] == self._in_channels * 2, (
                f"Expected {self._in_channels * 2} channels for single-tensor call, "
                f"got {inputs.shape[1]}."
            )
            x = inputs[:, :self._in_channels]
            d = inputs[:, self._in_channels:]

        b1, b2, b5 = self.encoder(x)
        d1, d2, d5 = self.encoder_d(d)

        output,   db3, db4 = self.decoder(b5)
        output_d, dd3, dd4 = self.decoder_d(d5)

        merge1   = self.mlp1(torch.cat((b1, db4), dim=1))
        merge2   = self.mlp2(torch.cat((b2, db3), dim=1))
        merge2   = F.interpolate(merge2, size=(self.img_size, self.img_size),
                                 mode='bilinear', align_corners=False)

        merge_d1 = self.mlp1_d(torch.cat((d1, dd4), dim=1))
        merge_d2 = self.mlp2_d(torch.cat((d2, dd3), dim=1))
        merge_d2 = F.interpolate(merge_d2, size=(self.img_size, self.img_size),
                                 mode='bilinear', align_corners=False)

        rgb1_w, dep1_w, _ = self.fusion1(merge1,  merge_d1)
        rgb2_w, dep2_w, _ = self.fusion2(merge2,  merge_d2)

        mask = self.mlp3(torch.cat((rgb1_w, rgb2_w, dep1_w, dep2_w), dim=1))

        return output, output_d, mask

    def log_fusion_weights(self, inputs, depth=None):
        """
        Debug helper: returns average fusion weights for the current batch.
        Useful for monitoring whether the depth branch is being utilized.

        Example usage in trainer (every N iterations):
            w1, w2 = model.log_fusion_weights(rgb, dep)
            print(f"fusion1  rgb={w1[0]:.3f}  dep={w1[1]:.3f}")
            print(f"fusion2  rgb={w2[0]:.3f}  dep={w2[1]:.3f}")
        """
        with torch.no_grad():
            if depth is not None:
                x, d = inputs, depth
            ...
            b1, b2, b5 = self.encoder(x)
            d1, d2, d5 = self.encoder_d(d)
            _, db3, db4 = self.decoder(b5)
            _, dd3, dd4 = self.decoder_d(d5)

            m1  = self.mlp1(torch.cat((b1, db4), dim=1))
            md1 = self.mlp1_d(torch.cat((d1, dd4), dim=1))
            m2  = self.mlp2(torch.cat((b2, db3), dim=1))
            md2 = self.mlp2_d(torch.cat((d2, dd3), dim=1))
            m2  = F.interpolate(m2,  size=(self.img_size, self.img_size),
                                mode='bilinear', align_corners=False)
            md2 = F.interpolate(md2, size=(self.img_size, self.img_size),
                                mode='bilinear', align_corners=False)

            _, _, w1 = self.fusion1(m1, md1)
            _, _, w2 = self.fusion2(m2, md2)

        return w1.mean(0).cpu().tolist(), w2.mean(0).cpu().tolist()
