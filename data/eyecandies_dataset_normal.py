...
class Eyecandies(Dataset):
    ...
    def load_and_convert_depth(self, depth_img):
...
class EyecandiesTrain(Eyecandies):
    ...
    def transform_image(self, image_path, depth_path, depth_info_path, anomaly_source_path, fgmask_path):
        ...
        depth_img = self.load_and_convert_depth(depth_path)
        depth_img = cv2.resize(depth_img, dsize=(self.resize_shape[1], self.resize_shape[0]), interpolation=cv2.INTER_NEAREST)
        normalized_depth_img = 1-(depth_img - np.min(depth_img)) / (np.max(depth_img)-np.min(depth_img))
        normalized_depth_img = np.expand_dims(normalized_depth_img, axis=2)
        ...
        if self.aug_type == "gaussian":
            augmented_image, normalized_augmented_depth, mask_image, mask_zzz, has_anomaly = self.augment_image_gaussian(rgb_img, normalized_depth_img, anomaly_source_path, fg_mask)
        else:
            augmented_image, normalized_augmented_depth, mask_image, mask_zzz, has_anomaly = self.augment_image(rgb_img, normalized_depth_img, anomaly_source_path, fg_mask)
        
        augmented_depth_img = (1-normalized_augmented_depth) * 65535.0

        normal_img = depth2normal(depth_img).astype(np.float32) * 0.5 + 0.5
        augmented_normal_img = depth2normal(augmented_depth_img[..., 0]).astype(np.float32) * 0.5 + 0.5

        # Adjusting dimensions
        rgb_img = np.transpose(rgb_img, (2, 0, 1))
        normal_img = np.transpose(normal_img, (2, 0, 1))
        augmented_image = np.transpose(augmented_image, (2, 0, 1))
        augmented_normal_img = np.transpose(augmented_normal_img, (2, 0, 1))
        ...
        return (rgb_img, augmented_image, normal_img, augmented_normal_img, has_anomaly, mask_image, mask_zzz)
...
class EyecandiesTest(Eyecandies):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'image': rgb_img,'mask': mask_img ,'has_anomaly': has_anomaly,'zzz': normal_img, "file_path": self.img_paths[idx]}
