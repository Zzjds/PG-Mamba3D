...
class MVTec3DTrain(MVTec3D):
    ...
    def augment_image_gaussian(self, image, anomaly_source_path, depth):
        ...
        if self.noise_type == 'gaussian':
            perlin_noise = np.random.randn(*perlin_noise.shape)
        elif self.noise_type == 'laplacian':
            perlin_noise = np.random.laplace(*perlin_noise.shape)
    ...
    def transform_image(self, image_path, tiff_path, anomaly_source_path):
        ...
        augmented_zzz = np.repeat(augmented_zzz, 3, axis=2)
        ...
        return (image, augmented_image, zzz, augmented_zzz, has_anomaly, mask_image, mask_zzz)
...
def get_data_loader(args, split, class_name, img_size,batch_size=1, num_workers=1,shuffle=False, is_fusion=False):
    if split in ['train']:
        if class_name == 'all':
            dataset = ConcatDataset([MVTec3DTrain(class_name=cls_name, img_size=img_size, dataset_path=args.data_dir,
                                anomaly_source_path=args.anomaly_source_path,fusion=is_fusion, args=args) for cls_name in mvtec3d_classes()])
        else:
